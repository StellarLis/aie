...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, min_missing_share: float = 0.5) -> Dict[str, Any]:
    ...
    # Есть ли колонки, где все значения одинаковые
    flags["has_constant_columns"] = any(column.unique == 1 for column in summary.columns)
    
    # Есть ли признаки с очень большим числом уникальных значения (порог = 0.9)
    flags["has_high_cardinality_categoricals"] = any((column.unique / column.non_null) >= 0.9 for column in summary.columns)

    # Уникальный ли признак user_id
    flags["has_suspicious_id_duplicates"] = any(column.name == "user_id" and column.non_null != column.unique for column in summary.columns)
