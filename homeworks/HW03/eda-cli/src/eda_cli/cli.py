...
def report(
    ...
    title: str = typer.Option("EDA-отчёт", help="Заголовок README.md отчёта"),
    top_k_categories: int = typer.Option(5, help="Количество top-значений для категориальных признаков"),
    min_missing_share: float = typer.Option(0.5, help="Порог доли пропусков, выше которого колонка считается проблемной")
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть дублирующиеся user_id: **{quality_flags['has_suspicious_id_duplicates']}**\n\n")
...
def head(
    ...
    df_head = _load_csv(Path(path), sep=sep, encoding=encoding).head(n)
    typer.echo(df_head.to_string(index=False))
